    ...
    public String getRouterControlIp(final long routerId) {
        ...
        for (final NicVO n : nics) {
            ...
            if (nc != null && nc.getTrafficType() == TrafficType.Control) {
                ...
                return VirtualMachineManager.getControlAddress(router.getHypervisorType(), n.getIPv4Address(), n.getMacAddress());
            }
        }

        logger.warn("Unable to find router's control ip in its attached NICs!. routerId: " + routerId);
        return VirtualMachineManager.getControlAddress(router.getHypervisorType(), router.getPrivateIpAddress(), router.getPrivateMacAddress());
    }
